...
export default function LoginPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const ok = [
      ...
      suConfirm.validate([
        rules.required("التأكيد"),
        rules.match(suPassword.value, "كلمة المرور"),
      ]),
    ].every(Boolean);
    if (!ok) return;
    ...
    if (error) {
      toast.error("فشل إنشاء الحساب", "حاول مرة أخرى أو تواصل مع الدعم");
      return;
    }

    // Supabase بيرجع identities فاضية لو الإيميل موجود مسبقاً
    if (
      data.user &&
      ...
      toast.error(
        "البريد مسجّل مسبقاً",
        "جرّب تسجيل الدخول أو استعادة كلمة المرور",
      );
      return;
    }
  ...
  const handleReset = async (e: React.FormEvent) => {
    ...
    if (error)
      toast.error("فشل الإرسال", "تأكد من البريد الإلكتروني وحاول مرة أخرى");
    else {
            ...
            {tab === "signup" && (
              <form onSubmit={handleSignUp} className="lp-form" noValidate>
                ...
                <Field
                  label="البريد الإلكتروني"
                  error={suEmail.error}
                  touched={suEmail.touched}
                >
                  <input
                    type="email"
                    placeholder="example@university.edu"
                    value={suEmail.value}
                    onChange={suEmail.onChange}
                    onBlur={suEmail.onBlur}
                    className={`lp-input ${suEmail.touched && suEmail.error ? "lp-input-err" : ""}`}
                  />
                </Field>
                <Field
                  label="كلمة المرور"
                  error={suPassword.error}
                  touched={suPassword.touched}
                >
                  <input
                    type="password"
                    placeholder="6 أحرف على الأقل"
                    value={suPassword.value}
                    onChange={suPassword.onChange}
                    onBlur={suPassword.onBlur}
                    className={`lp-input ${suPassword.touched && suPassword.error ? "lp-input-err" : ""}`}
                  />
                </Field>
                <Field
                  label="تأكيد كلمة المرور"
                  error={suConfirm.error}
                  touched={suConfirm.touched}
                >
                  <input
                    type="password"
                    placeholder="••••••••"
