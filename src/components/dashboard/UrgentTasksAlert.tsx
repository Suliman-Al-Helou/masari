...
export default function UrgentTasksAlert({
  urgentTasks: initialTasks,
}: UrgentTasksAlertProps) {
  ...
  const handlePostpone = async (id: string) => {
    ...
    try {
      ...
      /* silent — الـ UI يعكس التغيير والـ DB يتزامن لاحقاً */
    }
  };
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <span
                      className={`text-[10px] flex items-center gap-0.5 font-medium ${isOverdue ? "text-destructive" : "text-amber-600"}`}
                    >
                      <Clock className="w-3 h-3" />
                      {hours !== null
                        ? isOverdue
                          ? "انتهى الموعد!"
                          : `متبقي ${formatTimeLeft(hours)}`
                        : ""}
                    </span>
                  </div>
                </div>
